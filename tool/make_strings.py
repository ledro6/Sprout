...
# Строка без перевода хоть на один язык — ошибка. False ставят, когда
# переводы на паузе: новые строки в таблицы других языков не пишутся,
# система покажет русский текст, а уже переведённое по-прежнему
# проверяется целиком.
TRANSLATIONS_REQUIRED = True
