...
struct HomeView: View {
    ...
    /// Что сейчас открыто поверх главной. Держим путь сами, а не отдаём
    /// его целиком стеку: пока экран растения открыт, ореолы под
    /// карточками должны быть погашены, и знать об этом может только
    /// главная.
    ...
    var body: some View {
        NavigationStack(path: $open) {
            ScrollView {
                ...
                // Ореолы гаснут, пока открыт экран растения: тень и
                // свечение лежат отдельными размытыми слоями и в переходе
                // не перетекают вместе с карточкой, а тянутся за ней
                // хвостом. Флаг стоит здесь, а не на прокрутке, чтобы не
                // достаться заодно и самому экрану растения — у его
                // плашек тень своя и гасить её незачем.
                .environment(\.sproutHalos, open.isEmpty)
                // Только на возвращение. При открытии анимации нет: там
                // ореолы должны исчезнуть сразу, до первого кадра.
