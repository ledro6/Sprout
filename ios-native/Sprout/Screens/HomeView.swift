...
struct HomeView: View {
    ...
    ///
    /// Размер не задан числом: системная кнопка меряет себя по значку и
    /// своим полям — как соседние кнопки в панелях iOS. Задан крупный
    /// размер элемента управления и тот же кегль значка, что у кнопок на
    /// экране растения: со стандартным кнопка выходила заметно мельче их,
    /// а стоит она на том же месте экрана и делает то же — открывает.
    ///
    /// Значок при этом идёт за настройкой размера текста, а подпись
    /// комнаты рядом при прокрутке дорастает до заголовка — за ней знак
    /// настроек не растёт и остаётся знаком.
