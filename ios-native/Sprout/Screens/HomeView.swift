...
struct HomeView: View {
    ...
    ///
    /// Размер не задан: системная кнопка меряет себя сама по значку и
    /// своим полям — как соседние кнопки в панелях iOS. Значок идёт за
    /// настройкой размера текста, а вот подпись комнаты рядом при
    /// прокрутке дорастает до заголовка — знак настроек за ней не растёт
    /// и остаётся знаком.
    private var gear: some View {
        Button { settings = true } label: {
            Image(systemName: "gearshape")
                .font(Typography.barIcon)
