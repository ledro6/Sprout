...
struct Plant: Identifiable, Hashable, Codable {
    ...
    /// «Следующий полив: сегодня / завтра / 5 дней».
    ///
    /// С двоеточием: подпись живёт в одну строку, а двоеточие даёт глазу
    /// зацепку там, где иначе была бы длинная фраза без пауз. Оно же
    /// заменяет собой «через»: после двоеточия срок читается сроком и
    /// без предлога, а строка короче на шесть знаков.
...
enum Seed {
    /// Имени у нового сада нет.
    ///
    /// Раньше здесь стояло «Святослав» — имя из макета, — и оно встречало
    /// на заставке каждого, кто поставил приложение. Макет рисовали на
    /// примере одного человека, а приложение ставят все; имя хозяина
    /// берётся из профиля, а пока его не назвали, приложение здоровается
    /// без имени.
    static let owner = ""

    /// Как подписывать того, кто ещё не назвался, там, где без имени
    /// нельзя: в коде для друзей и в строке таблицы.
    static let stranger = "Садовод"

    /// Приветствие на заставке.
    ///
    /// Здесь, а не на самой заставке: строка зависит только от имени, и
    /// проверить её можно без телефона.
