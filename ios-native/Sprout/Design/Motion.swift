...
enum Motion {
    ...
    /// 1.2 с ореола нет вовсе — карточка складывается и встаёт на место, —
    /// потом он проявляется за полсекунды. При открытии гаснет сразу, без
    /// анимации.
    static let haloWait: Duration = .seconds(1.2)
    ...
    /// Гирлянда: огонь обегает свою длину за 2.6 с. Кадров — десять в
    /// секунду: ступеней яркости всего шесть, чаще рисовать нечего, а фон
    /// на каждом экране.
    static let garlandPeriod = 2.6
    static let garlandFrame = 0.1
    ...
    /// Сколько подержать тащимую карточку над чужим местом, чтобы соседи
    /// расступились, — как на «Домой». Проведённая через всю полку, она не
    /// тасует каждую, мимо которой прошла.
    static let arrangeDwell: Duration = .milliseconds(300)

    /// Листание к комнате нажатием: без отскока — страница не брошена, а
    /// подведена.
    static let page = Animation.smooth(duration: 0.42)

    /// Покачивание полки — как виджеты на «Домой»: поворот вокруг середины,
    /// такт один на всех, фаза у каждой карточки своя. Угол не общий, а от
    /// размера: край карточки ходит на столько же, на сколько край значка,
    /// — у крупной угол меньше. С общим углом в 1.2° край карточки уходил
    /// почти на три пункта, а с дрожью по восьмёрке поверх полка дёргалась.
    static let jiggleTravel: CGFloat = 1.8
    static let jigglePeriod = 0.32
