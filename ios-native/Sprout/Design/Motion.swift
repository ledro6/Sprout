...
enum Motion {
    ...
    /// Сад в AR попросили кнопкой действия, а доска готовности ещё пуста:
    /// при холодном запуске мастерская узнаёт готовые модели за миг, и
    /// пугать «моделями ещё нет» сразу незачем.
    static let summonGrace = 1.0
