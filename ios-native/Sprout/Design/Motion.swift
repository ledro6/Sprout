...
enum Motion {
    ...
    /// Полторы секунды ореола нет вовсе — карточка складывается, встаёт на
    /// место и успевает постоять, — потом он проявляется за полсекунды. При
    /// открытии гаснет сразу, без анимации.
    static let haloWait: Duration = .seconds(1.5)
