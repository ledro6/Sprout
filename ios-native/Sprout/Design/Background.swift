...
/// Подложка у верхнего края экрана — поверх содержимого.
///
/// Содержимое прокручивается под строкой состояния и там видно: у
/// ScrollView своя область обрезки, и безопасная зона ей не граница.
/// Что-то должно эту полосу закрывать.
///
/// Раньше закрывала растяжка, привязанная к строке комнаты и поднятая
/// над ней на 200 pt. Прокрученный экран это чинило, а спокойный портило:
/// в покое строка стоит ниже заголовка, и та же растяжка ложилась прямо
/// на слово «Главная» — чёрный текст под белой вуалью в девять десятых
/// читается почти белым. Привязка к содержимому и была ошибкой: полоса
/// нужна там, где стоит вырез, а не там, куда доехала строка.
///
/// Поэтому подложка стоит сама по себе, ровно над безопасной зоной.
/// Схода вниз у неё нет, и он не нужен: под ней либо закреплённая строка
/// со своей растяжкой, либо верх фона, а он в этой полосе и так сплошной.
/// Нулевая высота с выходом за безопасную зону — это и есть «ровно
/// строка состояния»: в разметке полоса не занимает ничего.
struct TopCover: View {
    var body: some View {
        Palette.background
