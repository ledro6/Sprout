...
private struct SproutPattern: View {
    ...
    ///
    /// Проход простой: фигурка раздалась и вернулась. Одна волна, один
    /// гребень.
    ///
    /// Было ямка — гребень — ямка, три полосы: поджатая, поднявшаяся и
    /// снова поджатая. На экране их и читалось три, а не одна волна.
    /// Поджим убран целиком; окно заодно сузилось втрое, до трёх делений
    /// сетки, — в широком соседний ряд поднимался почти вровень с
    /// гребнем, и размах приходилось держать мелким.
    private func pop(at middle: CGPoint) -> CGFloat {
        ...
        // Синус в квадрате: один горб, и на обоих концах окна он ноль не
        // только по высоте, но и по скорости. Фигурка не трогается с
        // места на полном ходу — волна до неё дошла, а та ещё стоит, — и
        // так же мягко возвращается к своему размеру.
        let swing = sin(.pi * step) * sin(.pi * step)
        return CGFloat(1 + Metrics.popAmp * swing)
    }
}
