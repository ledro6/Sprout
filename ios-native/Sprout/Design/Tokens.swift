...
enum Palette {
    ...
    static func swatch(_ hue: Hue) -> Color { colour(hue.vivid, 1) }

    /// Свой цвет — таким, как его выбрали в палитре, до притемнения.
    static func raw(_ c: Channels) -> Color { colour(c, 1) }
...
enum Metrics {
    ...
    /// Скругление барабана перед гранью — в кеглях: уходящее имя
    /// загибается влево круто и тает, не заслоняя текущее.
    static let roomCurl: CGFloat = 1.6

    /// Толчок смены комнаты — мягкий, едва глубже щелчка; на «Новой
    /// комнате» — почти шёпот, и гул оттяжки к ней втрое тише прежнего.
    static let roomThud = 0.4
    static let freshThud = 0.12
    static let freshHum = 0.3

    /// Сход у правого края: длинное следующее имя гаснет, а не обрывается.
    static let roomTail: CGFloat = 36

    /// Размытие соседа — доля кегля: 1.8 pt у подписи в покое, 2.7 у
    ...
    /// Гирлянда: бегущий огонь длиной в семь узлов сетки, а огонёк в стороне
    /// от него горит на четверть — совсем погасший читался бы каплей.
    static let garlandSpan = 7.0
    static let garlandFloor = 0.25
