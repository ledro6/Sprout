...
enum Metrics {
    ...
    /// Отступ шапки от безопасной зоны: 34 — полоса, которую раньше
    /// занимала плашка с логотипом, плюс 11 из макета. Плашка ушла под
    /// вырез и высоты больше не занимает, но приветствие стоит там же,
    /// где стояло.
    static let headerTop: CGFloat = 45

    /// Растяжка под закреплённой шапкой: полупрозрачная, последние 44 pt
    /// сходят на нет и свисают на 22 pt ниже шапки.
    ///
    /// Сплошной её делать нельзя — сквозь неё должен читаться узор, как
    /// сквозь панель в мессенджерах. Из-за прозрачности верх первого ряда
    /// карточек она слегка забеливает; это плата за плавный сход, и
    /// именно поэтому сход длинный, а не обрывается по низу шапки.
    static let headerWashOpacity: CGFloat = 0.82
    static let headerWashFade: CGFloat = 44
    static let headerWashOverhang: CGFloat = 22
