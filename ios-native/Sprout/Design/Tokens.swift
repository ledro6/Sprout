...
enum Metrics {
    ...
    /// Верх приветствия — тоже от верха экрана, а не от безопасной зоны:
    /// плашка привязана к экрану, и шапка должна вставать под ней
    /// одинаково при любом вырезе.
    ///
    /// В макете приветствие стоит на 77, здесь 73. Разница в кнопке
    /// комнаты: она занимает в потоке все свои 44 pt, тогда как в макете
    /// это площадь нажатия, заходящая за строку вверх и вниз. Забираем
    /// 4 pt, и строка комнаты с карточками встают ровно по макету.
    static let headerTop: CGFloat = 73

    /// Растяжка под закреплённой шапкой: сплошная, а последние 16 pt
    /// сходят в прозрачность.
    ///
    /// Эти 16 — весь запас, какой есть: 13 pt пустых внизу кнопки
    /// комнаты (её 44 pt — площадь нажатия, подпись занимает 18) плюс
    /// 3 pt до карточек. Длиннее растяжку тянуть некуда — она лежит
    /// поверх содержимого и начала бы белить верхний ряд.
    static let headerWashFade: CGFloat = 16
    static let headerWashOverhang: CGFloat = 3
