...
enum Metrics {
    ...
    /// Растяжка под строкой комнаты. Сплошная: под ней проходит строка
    /// состояния, и призрак карточки под часами читался бы поломкой.
    /// Проступает по мере прокрутки, поэтому заголовок в покое не глушит.
    static let headerWashOpacity: CGFloat = 1
    static let headerWashFade: CGFloat = 32

    /// Длина схода минус 13 pt, что пустуют под подписью комнаты (кнопка 44,
    /// подпись 18); на столько же отодвинуты карточки.
