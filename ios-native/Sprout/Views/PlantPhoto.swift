...
struct PlantPhoto: View {
    ...
    var body: some View {
        if let shot = plant.shot, let image = Snapshot.image(shot) {
            ...
            Color.clear
                // Запасной размер — на случай, когда размера не предложили
                // вовсе. Так бывает ровно в одном месте: в предпросмотре
                // контекстного меню система спрашивает у содержимого,
                // сколько ему нужно, а пустой цвет на такой вопрос
                // отвечает «десять на десять» — и снимок в меню исчезал.
                // Рисунок из ассетов не исчезал: `Image.resizable` без
                // предложения отвечает своим настоящим размером.
