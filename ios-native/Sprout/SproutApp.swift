...
struct RootView: View {
    ...
    private var cover: some View {
        // Один градиент сверху вниз: у самого верха сплошной цвет, к
        // низу полосы сходит в прозрачность. Ни ровной части, ни стыка —
        // стыку тут и взяться неоткуда: узор идёт под подложкой
        // сплошняком и проступает по мере того, как она тает.
        //
        // Две прошлые попытки бились именно о стык. Сперва подложка была
        // ровным цветом поверх узора, и он обрывался под её краем чертой
        // через весь экран. Потом в подложке рисовался второй фон, чтобы
        // узор совпал, — совпасть он не мог: у подложки своя разметка,
        // свой отсчёт и свой наклон от акселерометра.
        LinearGradient(
